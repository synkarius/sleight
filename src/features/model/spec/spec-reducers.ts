...
const specsSlice = createSlice({
  ...
  reducers: {
    ...
    changeSpecItemType: (
      state,
      action: PayloadAction<ChangeSpecItemTypePayload>
    ) => {
      if (state.editing) {
        const specItem = state.editing.items.find(
          (specItem) => specItem.id === action.payload.specItemId
        );
        if (specItem) {
    ...
    changeSpecItemVariableId: (
      state,
      action: PayloadAction<ChangeSpecItemVariableIdPayload>
    ) => {
      if (state.editing) {
        const specItem = state.editing.items.find(
          (specItem) =>
    ...
    changeSpecItemOrder: (
      state,
      action: PayloadAction<ChangeSpecItemOrderPayload>
    ) => {
      if (state.editing) {
        const specItem = state.editing.items.find(
          specItemIdMatches(action.payload.specItemId)
        );
        const specItemIndex = state.editing.items.findIndex(
          specItemIdMatches(action.payload.specItemId)
        );
        if (specItem) {
          const newIndex =
    ...
    deleteSpecItem: (state, action: PayloadAction<string>) => {
      if (state.editing && state.editing.items.length > 1) {
        state.editing.items = state.editing.items.filter(
          (specItem) => specItem.id !== action.payload
        );
      }
    },
  },
